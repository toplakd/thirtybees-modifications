                ...
                <div class="button-container">
                    {if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.customizable != 2 && !$PS_CATALOG_MODE}
                        {if (!isset($product.customization_required) || !$product.customization_required) && ($product.allow_oosp || $product.quantity > 0)}
                            ...
                            <a class="ajax_add_to_cart_button btn btn-primary btn-icon-cart"
                        ...
                        {else}
                            <span class="ajax_add_to_cart_button btn btn-primary disabled btn-icon-cart">
                              <i class="icon icon-shopping-basket"></i>
                            </span>
                        {/if}
                    {/if}
                    <a class="btn btn-default btn-icon-info" href="{$product.link|escape:'html':'UTF-8'}" title="{l s='View'}">
