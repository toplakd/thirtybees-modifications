...
function updateAddressId(id_product, id_product_attribute, old_id_address_delivery, id_address_delivery, line) {
  if (typeof(line) == 'undefined' || line.length == 0)
    line = $('#cart_summary div[id^=product_' + id_product + '_' + id_product_attribute + '_0_], #cart_summary div[id^=product_' + id_product + '_' + id_product_attribute + '_nocustom_]');
...
function updateCartSummary(json) {
      ...
      for (var i in json.discounts)
        if (json.discounts[i].id_discount == idElmt) {
          if (json.discounts[i].value_real !== '!') {
            if (priceDisplayMethod !== 0)
              $('#cart_discount_' + idElmt + ' div.cart_discount_price span.price-discount').html(formatCurrency(json.discounts[i].value_tax_exc * -1, currencyFormat, currencySign, currencyBlank));
            else
              $('#cart_discount_' + idElmt + ' div.cart_discount_price span.price-discount').html(formatCurrency(json.discounts[i].value_real * -1, currencyFormat, currencySign, currencyBlank));
          }
          toDelete = false;
        }
      if (toDelete)
  ...
  $('.cart_total_delivery').css('display', 'flex');
  if (json.total_shipping > 0) {
    if (priceDisplayMethod !== 0)
  ...
  $('#advanced_payment_options').show();
  if(json.errors.length !== 0) {
      $('.shipping_error > div').html(json.errors.join('<br>'));
      $('.shipping_error').show();
      $('.cart_total_delivery').hide();
      $('#advanced_payment_options').hide(); // alza added for Advanced OPC
      document.getElementById("cgv").checked = false; // added for Advanced OPC


  } else if($('.shipping_error:visible').length != 0) {
      $('.shipping_error').hide();
      $('.shipping_error > div').html('');
	  $('#advanced_payment_options').show(); // alza added for Advanced OPC
