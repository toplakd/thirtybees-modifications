...
{if !empty($list_item.children)}
  ...
  <div class="list-group-item-wrapper{if (isset($currentCategoryId) && $list_item.id == $currentCategoryId) || (!empty($list_item.expanded) && $list_item.expanded)} active{/if}">
    <a href="{$list_item.link|escape:'html':'UTF-8'}" class="mobile-list-group-item list-group-item ilvl-{$level|intval}{if isset($currentCategoryId) && $list_item.id == $currentCategoryId} current{/if}">
      <span>{$list_item.name|escape:'html':'UTF-8'}</span>
    </a>
    <a class="btn-toggle{if empty($list_item.expanded)} collapsed{/if} ilvl-{$level|intval}" href="#mobile{$list_item_id|escape:'html':'UTF-8'}" data-toggle="collapse" title="{l s='Expand/Collapse' mod='blockcategories'}">
      <i class="icon icon-minus-circle"></i>
    </a>
  </div>
  <div {if empty($list_item.expanded)} class="list-group collapse" style="height: 0px;"{else} class="list-group collapse in" style="height: auto;"{/if} id="mobile{$list_item_id|escape:'html':'UTF-8'}">
...
{else}
  <a class="mobile-list-group-item list-group-item ilvl-{$level|intval}{if isset($currentCategoryId) && $list_item.id == $currentCategoryId} active current{/if}" href="{$list_item.link|escape:'html':'UTF-8'}">
    <span>{$list_item.name|escape:'html':'UTF-8'}</span>
  </a>
{/if}
