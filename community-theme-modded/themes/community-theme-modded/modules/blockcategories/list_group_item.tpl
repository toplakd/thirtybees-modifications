...
{if !empty($list_item.children)}
  ...
  <div class="list-group-item-wrapper{if (isset($currentCategoryId) && $list_item.id == $currentCategoryId) || (!empty($list_item.expanded) && $list_item.expanded)} active{/if}">
    ...
    <a class="btn-toggle{if empty($list_item.expanded)} collapsed{/if} ilvl-{$level|intval}" data-target="#{$list_item_id|escape:'html':'UTF-8'}" data-toggle="collapse" title="{l s='Expand/Collapse' mod='blockcategories'}">
